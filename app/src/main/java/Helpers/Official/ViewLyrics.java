...
public class ViewLyrics {
    ...
    public static Lyrics fromMetaData(String artist, String title) throws IOException, NoSuchAlgorithmException, SAXException, ParserConfigurationException {
        ...
        int artistDistance = LevenshteinDistance.getDefaultInstance().apply(results.get(0).getArtist(), artist);
        int titleDistance = LevenshteinDistance.getDefaultInstance().apply(results.get(0).getTrack(), title);
        ...
        if(res!=null)
        result.setText(res.replaceAll("(\\[(?=.[a-z]).+]|<.+?>|www.*[\\s])", "").replaceAll("[\n\r]", " ").replaceAll("\\[", "\n\\["));
        else return new Lyrics(Lyrics.ERROR);
    ...
    private static ArrayList<Lyrics> search(String searchQuery) throws IOException, ParserConfigurationException, SAXException, NoSuchAlgorithmException {
        ...
        RequestBody body = RequestBody.create(MediaType.parse("application/text"), assembleQuery(searchQuery.getBytes(StandardCharsets.UTF_8.toString())));
    ...
    private static String getUrlAsString(String URl){
        try {
            return IOUtils.toString(URI.create(URl), StandardCharsets.UTF_8.toString());
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
